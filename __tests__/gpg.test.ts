...
const userInfos = [
  {
    ...
    fingerprints: [
      ...
      '5A282E1460C0BC419615D34DD523BD50DD70B0BA'
    ],
    keygrips: [
  ...
  },
  {
    ...
    keygrips: [
      'F5C3ABFAAB36B427FD98C4EDD0387E08EA1E8092',
      'DEE0FC98F441519CA5DE5D79773CB29009695FEB'
    ]
  }
]
  ...
  describe(userInfo.key, () => {
    ...
    describe('getKeygrip', () => {
      it('returns the keygrip for a given fingerprint', async () => {
        await gpg.importKey(userInfo.pgp)
        for (const {idx, fingerprint} of userInfo.fingerprints.map(
          (fingerprint, idx) => ({idx, fingerprint})
        )) {
    ...
    describe('presetPassphrase', () => {
      it('presets passphrase', async () => {
        ...
        for (const keygrip of await gpg.getKeygrips(userInfo.fingerprint)) {
          await gpg
            .presetPassphrase(keygrip, userInfo.passphrase)
            .then(output => {
              expect(output).not.toEqual('')
            })
        }
...
describe('parseKeygripFromGpgColonsOutput', () => {
  it('returns the keygrip of a given fingerprint from a GPG command output using the option: --with-colons', async () => {
    const outputUsingTestKey = fs.readFileSync(
    ...
    const keygripPrimaryTestKey = parseKeygripFromGpgColonsOutput(
      outputUsingTestKey,
      '27571A53B86AF0C799B38BA77D851EB72D73BDA0'
    )
    expect(keygripPrimaryTestKey).toBe(
      '3E2D1142AA59E08E16B7E2C64BA6DDC773B1A627'
    )

    const keygripSubkeyTestKey = parseKeygripFromGpgColonsOutput(
      outputUsingTestKey,
      '5A282E1460C0BC419615D34DD523BD50DD70B0BA'
    )
    expect(keygripSubkeyTestKey).toBe(
      'BA83FC8947213477F28ADC019F6564A956456163'
    )

    const outputUsingTestSubkey = fs.readFileSync(
    ...
    const keygripPrimaryTestSubkey = parseKeygripFromGpgColonsOutput(
      outputUsingTestSubkey,
      '87F257B89CE462100BEC0FFE6071D218380FDCC8'
    )
    expect(keygripPrimaryTestSubkey).toBe(
      'F5C3ABFAAB36B427FD98C4EDD0387E08EA1E8092'
    )

    const keygripSubkeyTestSubkey = parseKeygripFromGpgColonsOutput(
      outputUsingTestSubkey,
      'C17D11ADF199F12A30A0910F1F80449BE0B08CB8'
    )
    expect(keygripSubkeyTestSubkey).toBe(
      'DEE0FC98F441519CA5DE5D79773CB29009695FEB'
    )
  })
})
