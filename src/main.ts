...
async function run(): Promise<void> {
  try {
    ...
    await core.group(`GnuPG info`, async () => {
      core.info(
        `Version    : ${version.gnupg} (libgcrypt ${version.libgcrypt})`
      )
    ...
    if (inputs.passphrase && !inputs.fingerprint) {
      ...
      await core.group(`Getting keygrips`, async () => {
        for (const keygrip of await gpg.getKeygrips(fingerprint)) {
          ...
          await gpg
            .presetPassphrase(keygrip, inputs.passphrase)
            .then(stdout => {
              core.debug(stdout)
            })
        }
    ...
    if (inputs.passphrase && inputs.fingerprint) {
      ...
      await core.group(`Getting keygrip for fingerprint`, async () => {
        ...
        core.info(
          `Presetting passphrase for key ${fingerprint} with keygrip ${keygrip}`
        )
    ...
    if (inputs.trustLevel) {
      await core.group(`Setting key's trust level`, async () => {
        await gpg
          .setTrustLevel(privateKey.keyID, inputs.trustLevel)
          .then(() => {
            core.info(
              `Trust level set to ${inputs.trustLevel} for ${privateKey.keyID}`
            )
          })
      })
    }
    ...
    if (inputs.gitUserSigningkey) {
      ...
      if (userEmail != privateKey.email) {
        core.setFailed(
          `Committer email "${inputs.gitCommitterEmail}" (name: "${inputs.gitCommitterName}") does not match GPG private key email "${privateKey.email}" (name: "${privateKey.name}")`
        )
        return
      }
